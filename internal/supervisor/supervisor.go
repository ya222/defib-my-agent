...
// adoptSessionRef stores a parsed session ref before any resume, per the
// session strategy: only when none is known yet.
...
// buildCommand chooses start vs resume per the session strategy
// (docs/providers.md#session-strategy-important): the first attempt of an
// existing session resumes; a known ref after any attempt resumes when the
// provider supports it; otherwise a fresh start passes the (possibly
// pre-generated) ref through.
