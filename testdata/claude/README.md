...
| `usage-limit.stdout.log` | **Real text, reconstructed envelope.** The subscription-cap surface text and metadata are a **real capture** from a `claude 2.1.201` run that hit the weekly cap (`"You've hit your weekly limit · resets 4am (Europe/London)"`, `error:"rate_limit"`, HTTP `429`; the 5-hour cap uses the same shape with `"session limit"`). Recorded in a session transcript, not from a `claude -p` invocation, so the surrounding headless result-event envelope is modelled on the real `auth-error.stdout.log` capture (same `<synthetic>` assistant message + `is_error`/`api_error_status` result event). Drives `claude.usage_limit` → `SESSION_LIMIT` with a `clock_time` Reset Time. Replace with a verbatim `claude -p` capture if one is ever obtained. |
| `*.documented.stdout.log` (rate-limit-429, credit-low, overloaded-529) | **Not captured.** Per-minute rate limits, low credit, and 529 overloads cannot be triggered on demand without spending/exhausting real quota. These fixtures use the *captured* result-event shape (from `auth-error.stdout.log`) filled with Anthropic's documented error formats (`api_error_status`, `overloaded_error`, the documented low-credit message). Replace each with a real capture when one occurs — tracked in the repo issue "Replace documented-format Claude fixtures with real captures". |
...
The three remaining `*.documented.stdout.log` fixtures (rate-limit-429, credit-low,
overloaded-529) still await real captures. You don't need to run `claude` by hand — defib
already writes each attempt's raw stdout/stderr under `<state>/tasks/<task-id>/attempts/<n>/`.
So the next time a real limit occurs, copy the offending attempt's files straight in and record
its exit code:

```sh
defib status <task>          # find the attempt <n> that hit the limit + its exit code
A=~/.local/state/defib/tasks/<task-id>/attempts/<n>
cp "$A/stdout.log" testdata/claude/rate-limit-429.stdout.log   # or credit-low / overloaded-529
cp "$A/stderr.log" testdata/claude/rate-limit-429.stderr.log
```
...
table above and `docs/detection.md`. (`usage-limit.stdout.log` was already reconciled against a
real `claude 2.1.201` cap event — see its provenance row; the surface text is real even though
the headless envelope was reconstructed.)
